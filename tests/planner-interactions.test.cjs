...
await run('initMap()');await tick();assert.equal(run('state.map.mapTypeId'),'cream_pastel');assert.equal(run("state.map.mapTypes.get('cream_pastel').styles.find(s=>s.featureType==='water' && s.elementType==='geometry').stylers[0].color"),'#cfe4e9');
...
await run('renderEditorMap(null,state.testPlaces)');assert.equal(run('state.editorMap.mapTypeId'),'cream_pastel');assert.equal(run('state.editorResultMarkers.length'),2);
...
// Today's clock selects the active stop, always follows the active schedule.
...
Object.defineProperty(w.document,'hidden',{value:false,configurable:true});run('selectStop("later")');assert.equal(run('state.selectedId'),'later');run('state.clockMinute=null;tickTripClock()');assert.equal(run('state.selectedId'),'now');assert.equal(run('state.followClock'),true);assert.equal(el('#followCurrentSchedule'),null);
console.log('PASS: appearance selection and save payload, anchored popup, ON AIR timeline and mandatory live tracking.');
