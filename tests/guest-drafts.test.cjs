...
(async()=>{
 ...
 assert.equal(api.db.mt_trips.size,1);assert.equal(api.db.mt_itinerary_items.size,1);assert.equal(api.db.mt_itinerary_items.get('item-a').created_by,'user-a');assert.equal(api.db.mt_trips.get('trip-a').travelers[0].appearance.hair,'white');
