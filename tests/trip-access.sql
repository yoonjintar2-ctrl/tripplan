...
begin
 ...
 update public.mt_itinerary_items set name='editor success' where id=iid;
 ...
 select count(*) into n from public.mt_itinerary_items where id=iid and item_date='2026-11-02' and memo='keep memo' and cost_won=12000 and name='editor success';
 ...
 perform set_config('request.jwt.claims',jsonb_build_object('sub',guest_uid,'role','authenticated','is_anonymous',false)::text,true);
 update public.mt_itinerary_items set name='should not edit' where id=iid;
 get diagnostics n=row_count;
